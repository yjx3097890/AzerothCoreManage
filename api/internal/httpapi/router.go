...
func (s *Server) Router() *gin.Engine {
	...
	authed.Use(AuthRequired(s.app.Cfg.Panel.JWTSecret))
	{
		...
		authed.GET("/mybots/characters/:id/jobs/:jobId", s.mybotsGetJob)
		authed.POST("/mybots/characters/:id/jobs/:jobId/pause", RequireRole(RoleGM), s.mybotsPauseJob)
		authed.POST("/mybots/characters/:id/jobs/:jobId/resume", RequireRole(RoleGM), s.mybotsResumeJob)
		...
		authed.GET("/modules/checkpoints", RequireRole(RoleSuperAdmin), s.listModuleCheckpoints)
		authed.POST("/modules/checkpoints", RequireRole(RoleSuperAdmin), s.createModuleCheckpoint)
		authed.POST("/modules/checkpoints/stream", RequireRole(RoleSuperAdmin), s.createModuleCheckpointStream)
		authed.GET("/modules/checkpoints/:id", RequireRole(RoleSuperAdmin), s.getModuleCheckpoint)
		authed.POST("/modules/checkpoints/:id/rollback", RequireRole(RoleSuperAdmin), s.rollbackModuleCheckpoint)
		authed.DELETE("/modules/checkpoints/:id", RequireRole(RoleSuperAdmin), s.deleteModuleCheckpoint)
